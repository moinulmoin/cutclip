...
class ClipService: ObservableObject {
    ...
    func clipVideo(inputPath: String, job: ClipJob) async throws -> String {
        guard let ffmpegPath = binaryManager.ffmpegPath else {
            ...
            pipe.fileHandleForReading.readabilityHandler = { handle in
                ...
                if !data.isEmpty {
                    ...
                    if totalDuration == nil {
                        totalDuration = self.parseDuration(from: output)
                    }
                    
                    // Parse progress
                    if let duration = totalDuration,
                       let currentTime = self.parseCurrentTime(from: output) {
    ...
    private func generateOutputFileName(for job: ClipJob) -> String {
    ...
    func convertTimeToSeconds(_ timeString: String) -> Double? {
